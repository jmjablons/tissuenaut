...
d_morphology_nrandom %>%
  ggplot(aes(x = class, y=value, group = class, fill = class)) +
  geom_boxplot(outlier.colour = NA)+
  #geom_quasirandom(pch = 21) +
  geom_beeswarm(pch = 21, cex = 2)+
...
wilcox = d_morphology_nrandom %>%
  #filter(class %in% c("strong_stain2","painter")) %>%
  group_by(name) %>%
  rstatix::wilcox_test(value~class))

writexl::write_xlsx(
  stat, 
  paste0("blobs_analysis/result/stat_morphology_6n_random.xlsx"))
